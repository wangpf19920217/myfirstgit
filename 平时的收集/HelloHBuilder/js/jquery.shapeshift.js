...
(function() {
  ...
  (function($, window, document) {
    var Plugin, defaults, pluginName; //方法  参数  方法名
    pluginName = "shapeshift";  //方法名
    defaults = {  			//参数拓展
      selector: "*",		//查找元素
      enableDrag: true,		//准许拖拽
      enableCrossDrop: true,	//准许交叉替换
      enableResize: true,
      enableTrash: false,  //准许删除
      align: "center",		
      colWidth: null,
      columns: null,
      minColumns: 1,
    ...
    Plugin = (function() {  	//定义方法

      function Plugin(element, options) {  //两个参数   对象 拓展参数
        this.element = element;		//原生对象	
        this.options = $.extend({}, defaults, options);
        this.globals = {};					//全局
        this.$container = $(element);//jq对象
        if (this.errorCheck()) {// 1. 如果没有错误  则执行init();
          this.init();
        }
      }

      Plugin.prototype.errorCheck = function() {  //1.验证是否可用的方法  
        var $children, error_msg, errors, options;
        options = this.options;
        errors = false;
        error_msg = "Shapeshift ERROR:";
        if (options.colWidth === null) {  //如果这个参数为空   则查找所有子元素
          $children = this.$container.children(options.selector);
          if ($children.length === 0) {  //如果没有子元素 则当错误执行 
            errors = true;
      ...
      Plugin.prototype.init = function() {  // 2. 进入执行初始化
        this.createEvents(); //创建
        this.setGlobals();
        this.setIdentifier();
      ...
      Plugin.prototype.createEvents = function() {
        var $container, options, // 引入绑定的对象  和参数
          _this = this;
        options = this.options;
        $container = this.$container;		//arrange:安排    先解除 后绑定
